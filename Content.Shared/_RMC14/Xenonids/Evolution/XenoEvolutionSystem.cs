...
public sealed class XenoEvolutionSystem : EntitySystem
{
    ...
    // How long a raffle winner has to meet a fixable requirement (e.g. heal to full) before the slot is passed on.
    private readonly TimeSpan _raffleGracePeriod = TimeSpan.FromSeconds(30);

    // How often to remind a leapfrogger that an open higher slot went unclaimed because a lower tier is full.
    ...
    private void OnXenoEvolveDoAfter(Entity<XenoEvolutionComponent> xeno, ref XenoEvolutionDoAfterEvent args)
    {
        ...
        if (!valid)
        {
            // A raffle winner whose wind-up was interrupted or became invalid: release the reserved slot.
