...
/// <summary>
/// Spawns fire and smoke particle effects on entities that are on fire.
/// </summary>
public sealed class FlammableParticleSystem : EntitySystem
{
    ...
    private static readonly ProtoId<ParticleEffectPrototype> FireEffect  = "RMCFireContinuous";
    private static readonly ProtoId<ParticleEffectPrototype> SmokeEffect = "RMCFireSmoke";
    ...
    private void OnAppearanceChange(Entity<FlammableComponent> ent, ref AppearanceChangeEvent args)
    {
        ...
        if (state.OnFire && state.FireEmitter != null)
        {
            ...
            if (state.FireEmitter != null)  state.FireEmitter.Intensity  = intensity;
            if (state.SmokeEmitter != null) state.SmokeEmitter.Intensity = intensity;
        }
    }
